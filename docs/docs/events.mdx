...
sidebar_position: 4
title: Events
description: Fan-out in-process event publishing with IEmitter, IEventHandler, and sequential or concurrent orchestration.
---
...
Or from within a handler, by injecting `IEmitter`:
...
public class CreateTaskHandler : IRequestHandler<CreateTaskCommand, Guid>
{
    ...
    public CreateTaskHandler(IEmitter emitter) => _emitter = emitter;

    public async ValueTask<Result<Guid>> HandleAsync(CreateTaskCommand cmd, CancellationToken ct = default)
    {
        var id = Guid.NewGuid();
        // ... persist ...
        await _emitter.EmitAsync(new TaskCreatedEvent(id, cmd.Title), ct);
...
| `EmitMode.Outbox`  | Stores the event in the outbox. Dispatched when `IOutboxCommit.CommitAsync` is called.                        |
...
- [Context](./context) — the flow state an event handler can read, and why publishing is not part of it.
