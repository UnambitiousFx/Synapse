...
| ID | When it fires | Fix |
|---|---|---|
| `SYN101` | A request type (`IRequest` or `IRequest<TResponse>`) declared in this assembly has no handler in this assembly. Events and stream requests are not checked. A contracts-only assembly (requests without handlers) warns once per request, so set `dotnet_diagnostic.SYN101.severity = none` there. | Add a handler class with the matching `[RequestHandler...]` attribute, or disable the rule if the handler lives in another project. |
| `SYN102` | A `[PipelineBehavior]` behavior (or an `[assembly: SynapseGlobalBehavior(...)]` entry) applies to no handler visible from this assembly (for example its generic constraint matches no request), so it never runs. A `[PipelineBehavior]` in a shared library with no handlers of its own is inert in the consuming host too (see [modular monolith](./modular-monolith)), so the warning is real. Global entries whose behavior type comes from a referenced assembly are not reported. | Widen or correct the constraint, or delete the behavior. For a shared library, declare `[assembly: SynapseGlobalBehavior(typeof(...))]` in the composition root instead. |
| `SYN103` | A handler attribute is placed on a `record`. The generator ignores records, so the handler is not registered. | Declare the handler as a `class`. |
...
Disable a rule per project or per folder with an `.editorconfig` entry:
...
- `SYN101` only sees the current assembly. A request whose handler is in another project needs the rule disabled in the project that declares the request.
- `SYN104` cannot see manual registration. A handler you register by hand with `RegisterRequestHandler` or `RegisterEventHandler` is still reported when it has no attribute; silence it per class with `#pragma warning disable SYN104`, or disable the rule.
