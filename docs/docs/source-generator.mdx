...
`UseEventDispatcherRegistration` is **required** when publishing with `PublishAot=true` or full trimming. Without it, event dispatch falls back to runtime `typeof(T)` resolution which the AOT compiler cannot analyze.
:::
