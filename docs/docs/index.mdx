...
- **Result-based errors** — handlers return `Result` / `Result<T>` from `UnambitiousFx.Functional`; errors are values, not exceptions.
...
<Tabs>
  <TabItem value="cli" label=".NET CLI" default>
    ...
    dotnet add package UnambitiousFx.Synapse.Endpoints    # optional — endpoint-per-class HTTP layer
    ```
  </TabItem>
  <TabItem value="packageref" label="PackageReference">
    ```xml
    <PackageReference Include="UnambitiousFx.Synapse.Abstractions" Version="*" />
    <PackageReference Include="UnambitiousFx.Synapse" Version="*" />
    <!-- optional -->
    <PackageReference Include="UnambitiousFx.Synapse.AspNetCore" Version="*" />
    <PackageReference Include="UnambitiousFx.Synapse.Generator" Version="*" />
...
| `UnambitiousFx.Synapse.AspNetCore`   | `IHttpInvoker`, `IMvcInvoker`, `UseSynapsePropagation` middleware.        |
| `UnambitiousFx.Synapse.Generator`    | Roslyn source generator that eliminates manual handler registration. |
| `UnambitiousFx.Synapse.Endpoints`    | Endpoint-per-class HTTP layer with generated binding; bundles its own analyzer. See [Endpoints](./endpoints/). |
