...
Keep every pipeline behavior registration **closed** as well. `UseDefaultServiceProvider(o => o.ValidateOnBuild = false)` is not a way around an open-generic registration that MS DI rejects under AOT: `ValidateOnBuild` governs the startup descriptor sweep only, and the same check runs again on per-request resolution — see [Why `ValidateOnBuild = false` does not rescue an open-generic registration](./pipelines#why-validateonbuild--false-does-not-rescue-an-open-generic-registration).
