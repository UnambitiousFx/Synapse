...
sidebar_position: 8
title: Validation
description: Run validators before the handler is invoked using IRequestValidator and the built-in RequestValidationBehavior pipeline stage.
---
...
:::tip
If any validator returns `Result.Failure`, the **handler is never called** and the failure is returned directly to the caller.
:::
