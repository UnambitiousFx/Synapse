...
Behaviors run in **registration order** — the first behavior registered is the outermost wrapper. Register logging before validation so the log captures the full round-trip including validation failures.
:::
