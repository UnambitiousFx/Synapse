...
### Why `ValidateOnBuild = false` does not rescue an open-generic registration

If you do register an open generic at runtime and publish with `PublishAot=true`, MS DI refuses to
close it over a value-type response:
...
The instinctive workaround is to switch off the container's build-time validation:

```csharp
// Hides the startup AggregateException. Does NOT fix the failure.
builder.Host.UseDefaultServiceProvider(o => o.ValidateOnBuild = false);
```

That is a false fix. `ValidateOnBuild` governs **only** the eager sweep `BuildServiceProvider()`
performs over every registered descriptor. The AOT open-generic check
(`CallSiteFactory.VerifyOpenGenericAotCompatibility`) also runs on the normal per-request resolution
path, gated on `RuntimeFeature.IsDynamicCodeSupported` alone — which is `false` in any project built
with `PublishAot=true`, including a plain `dotnet run`. Disabling validation therefore moves the same
exception from startup to the first request that resolves the behavior: the app appears healthy and
every affected endpoint returns a `500`.

Emit closed registrations instead — `[PipelineBehavior]`, or
[`[assembly: SynapseGlobalBehavior]`](#registering-a-behavior-you-dont-own-eg-from-a-nuget-package)
for a behavior you don't own — and the check is never reached. See
[known issue 002](https://github.com/UnambitiousFx/Synapse/blob/main/docs/known-issues/002-validateonbuild-does-not-suppress-aot-open-generic-check.md)
