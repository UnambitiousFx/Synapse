...
| `SYN002` | Error | Two or more handlers are registered for one request type. Only the first is used; the others are silently ignored. Events are exempt: several handlers per event is normal. | Keep one handler per request. |
