...
description: Install Synapse, wire it up in a .NET application, and send your first command end-to-end.
---
...
<Tabs>
  ...
  <TabItem value="packageref" label="PackageReference">
    ```xml
    <PackageReference Include="UnambitiousFx.Synapse" Version="*" />
    ```
  </TabItem>
</Tabs>
...
:::note Prerelease and preview builds
`UnambitiousFx.Synapse.Endpoints` is published to nuget.org as a **prerelease only** — add it with
`dotnet add package UnambitiousFx.Synapse.Endpoints --prerelease`. Every merge to `main` also
pushes a `X.Y.Z-preview.0.N` build of all packages to
[GitHub Packages](https://github.com/UnambitiousFx/Synapse/pkgs/nuget); see the
[README](https://github.com/UnambitiousFx/Synapse#prerelease-packages) for the feed setup.
:::
...
Use the [Source Generator](./source-generator) to eliminate boilerplate in larger projects.
:::
...
:::tip
Handlers receive the command and a `CancellationToken`. Return `Result<TResponse>` for domain errors — **never throw**. See [Error Handling](./error-handling) for patterns.
:::
...
// Option 3 — TryGetValue (value only, ignore the error)
