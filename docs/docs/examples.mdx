...
| [`EndpointsApi`](https://github.com/UnambitiousFx/Synapse/tree/main/examples/EndpointsApi) | ASP.NET Core + Native AOT | Every [Endpoints](./endpoints/) shape: all four high-level base classes (including `ContractEndpoint`), all three low-level tiers, every binding rule, every response mapper, groups, streaming from a body and without one, a computed route, a custom verb, and a pipeline behaviour. See its [README](https://github.com/UnambitiousFx/Synapse/tree/main/examples/EndpointsApi#readme) for the shape-to-file map |
