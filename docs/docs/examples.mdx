...
| [`MinimalApi.Counter`](https://github.com/UnambitiousFx/Synapse/tree/main/examples/MinimalApi.Counter) | Class library (composed by MinimalApi) | Cross-assembly `RegisterGroup` composition; value-type (`int`) response under AOT; `[PipelineBehavior]` in a separate assembly |
