...
public static class EndpointHarness
{
    ...
    ///     The Synapse.Endpoints analyzer did not run for the assembly declaring <typeparamref name="TEndpoint" />,
    ///     so no route metadata was registered for it. This is the most likely first-run failure —
    ///     verify the analyzer is enabled for that assembly and that analyzers are not disabled for the
    ///     build.
    /// </exception>
        ...
        where TEndpoint : EndpointBase, new()
    {
        ...
        // DiagnosticSource base type some middleware asks for instead) is otherwise unresolved. Owned
        // by options rather than created as a throwaway local, so EndpointHarness<TEndpoint> can take
        // ownership of the same instance and dispose it - DI never disposes a pre-built singleton.
        options.Services.AddSingleton(options.DiagnosticListener);
        options.Services.AddSingleton<DiagnosticSource>(options.DiagnosticListener);
        ...
        return new EndpointHarness<TEndpoint>(provider, pipeline, routeDescription, options.DiagnosticListener);
    }
}
