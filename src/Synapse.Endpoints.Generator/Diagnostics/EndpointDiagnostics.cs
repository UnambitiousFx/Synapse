...
internal static class EndpointDiagnostics
{
    ...
    ///     the generator cannot reopen it to emit its route metadata and binding. Reported per type
    ///     that is missing the keyword, so a nested endpoint's error names the type the author must
    ///     actually change.
    /// </summary>
    /// <remarks>
    ...
    ///     see docs/docs/endpoints/reference/preview.mdx.
    /// </remarks>
    internal static readonly DiagnosticDescriptor EndpointMustBePartial = new(
        "SYNE020",
        "Endpoint must be declared partial",
        "'{0}' must be declared 'partial' so the analyzer can emit its generated members into it",
        Category,
        DiagnosticSeverity.Error,
        isEnabledByDefault: true);
